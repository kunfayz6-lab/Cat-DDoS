...
def countdown(t):
    ...
    while True:
        ...
        if remaining_time > 1:
            ...
            print()
            stdout.write(f"\r{Fore.RED}[*] {Fore.BLUE}Sending request {url} {Fore.CYAN}Sec {remaining_time:.2f}")
        else:
            stdout.flush()
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=conn) as session:
        ...
        async def progress():
            while time.time() < end_ts:
                ...
                print()
                stdout.write(f"\r{Fore.RED}[*] {Fore.BLUE}Sending request {url} {Fore.CYAN}Sec {remaining:.2f}")
...
if __name__ == "__main__":
    ...
    while attemps < 100:
        ...
        username = input(f"{Fore.CYAN}└> Username please:  \033[30m")
        print(f"{Fore.GREEN}┌[Cyber Attack Team")
        password = input(f"{Fore.GREEN}└> Your password:  \033[30m")
