...
def print_summary(url: str, duration: int, concurrency: int, method: str, rate_limit: float | None, summary: dict):
    ...
    print(f"{Fore.CYAN}╚{'═' * 50}╝")
    print(f"{Fore.GREEN}║ ╚••⟩ Target     : {url.ljust(50)}║")
    print(f"{Fore.GREEN}║ ╚••⟩ Duration   : {str(duration)+'s':<50}║")
    print(f"{Fore.GREEN}║ ╚••⟩ Concurrency: {str(concurrency):<50}║")
    print(f"{Fore.GREEN}║ ╚••⟩ Method     : {method:<50}║")
    print(f"{Fore.GREEN}║ ╚••⟩ Rate limit : {('None' if not rate_limit else str(rate_limit)+' req/s'):<50}║")
    print(f"{Fore.CYAN}|{'═' * 50}|")
